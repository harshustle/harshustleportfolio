...
function VerifyPayment() {
    ...
    const handleVerify = async () => {
        ...
        setError(isHin ? 'phone number humaare records me nahi mila.' : 'phone number not found in our records.'); setShowFallback(true); setVerifying(false);
    };
        ...
        <div style={{ background: 'var(--c-bg)', minHeight: '100vh', paddingTop: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.3s ease' }}>
            <div style={{ maxWidth: '440px', width: '100%', margin: '0 auto', padding: '2rem 1.5rem' }}>
                <div style={{ border: `1px solid var(--c-border)`, borderRadius: '16px', padding: '2.5rem', background: 'var(--c-bg-subtle)' }}>
                    ...
                    <h1 style={{ fontWeight: 900, color: 'var(--c-text)', fontSize: '1.5rem', letterSpacing: '-0.02em', marginBottom: '0.5rem' }}>{isHin ? 'payment verify karein' : 'verify payment'}</h1>
                    <p style={{ fontSize: '0.875rem', color: 'var(--c-text-muted)', marginBottom: '2rem', lineHeight: 1.6 }}>{isHin ? 'apni haliya payment ko verify karne aur humse judne ke liye apna registered phone number dalein.' : 'enter your registered phone number to verify your recent payment and connect with us.'}</p>
                    ...
                    <div style={{ marginBottom: '1.25rem' }}>
                        ...
                        <input type="text" value={inputPhone} onChange={e => setInputPhone(e.target.value)} placeholder={isHin ? 'phone number dalein' : 'enter phone number'} style={inputStyle} onKeyDown={e => e.key === 'Enter' && handleVerify()} />
                    ...
                    <button onClick={handleVerify} disabled={verifying} style={{ width: '100%', padding: '0.85rem', background: 'var(--accent)', border: 'none', color: '#fff', fontWeight: 700, borderRadius: '8px', cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit', opacity: verifying ? 0.7 : 1 }}>
                        {verifying ? (isHin ? 'verify ho raha hain...' : 'verifying...') : (isHin ? 'verify karein aur judein →' : 'verify & connect →')}
                    </button>

                    {showFallback && (
                        <div style={{ marginTop: '1.5rem', padding: '1.25rem', border: `1px solid var(--c-border)`, borderRadius: '10px' }}>
                            <p style={{ fontSize: '0.8rem', color: 'var(--c-text-muted)', marginBottom: '0.75rem', lineHeight: 1.6 }}>{isHin ? 'kya abhi bhi dikkat aa rahi hai? humse sidhe whatsapp par sampark karein.' : 'still having issues? contact us directly on whatsapp.'}</p>
                            <button onClick={() => { const msg = `Hello Support, I made a payment using phone number ${inputPhone} but I can't verify it. Can you please check?`; window.open(`https://wa.me/917839661372?text=${encodeURIComponent(msg)}`, '_blank'); }}
                                ...
                                {isHin ? '💬 support se whatsapp par chat karein' : '💬 chat with support on whatsapp'}
                            </button>
                        </div>
                    )}
                    <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
                        <Link to="/" style={{ fontSize: '0.78rem', color: 'var(--c-text-ghost)' }}>{isHin ? 'home par wapas jayein' : 'return to home'}</Link>
                    </div>
                </div>
            </div>
        </div>
